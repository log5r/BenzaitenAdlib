...
def corrected_note_num_list(notenumlist, chord_prog, remove_suffix_prob, strict_mode):
    ...
    # 最後の音（2部音符＋8部音符だけ持続）
    lnmp = list(map(lambda ln: [abs(ln - fixed_note_num_list[-1]), ln - fixed_note_num_list[-1]], [48, 52, 60, 64, 72, 76]))
...
def generate_files(model_idf, remove_suffix_prob, strict_mode=False):
    ...
    # 補正
    corrected_notes = corrected_note_num_list(note_num_list, chord_prog, remove_suffix_prob, strict_mode)
